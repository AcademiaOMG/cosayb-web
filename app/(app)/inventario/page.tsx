...
export default function InventarioPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(false)
    try {
      const res = await fetch(`${API}/api/v1/ingredients`, {
        credentials: "include",
      })
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/v1/organizations/me`, { credentials: "include" })
      ...
      .then((b) => {
        if (b?.data?.plan) setPlan(b.data.plan)
      })
      .catch(() => {})
  }, [load])
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          {form.costPerUnit &&
            form.weightGrams &&
            parseFloat(form.weightGrams) > 0 && (
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                Costo por gramo calculado:{" "}
                <strong>
                  $
                  {(
                    parseFloat(form.costPerUnit) / parseFloat(form.weightGrams)
                  ).toFixed(4)}
                </strong>{" "}
