...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    await authClient.signIn.social({
      provider: "google",
      // Ajusta esta ruta a tu frontend en producción
      callbackURL: `${process.env.NEXT_PUBLIC_FRONTEND_URL || "http://localhost:3001"}/onboarding`,
    });
  };
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      await authClient.signIn.email({
        email,
        password,
        // Reutiliza la misma lógica de redirección dinámica que Google
        callbackURL: `${process.env.NEXT_PUBLIC_FRONTEND_URL || "http://localhost:3001"}/onboarding`,
      });
    } catch (error) {
      ...
      <div className="max-w-md w-full bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-gray-800 z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">
            Accede al panel de la Academia
          </p>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 font-semibold py-3 px-4 rounded-xl hover:bg-gray-100 transition-all mb-6 active:scale-[0.98]"
        >
        ...
        {/* Formulario de Email (Desactivado temporalmente en la lógica, visualmente activo) */}
        <form onSubmit={handleEmailLogin} className="space-y-5">
          ...
          <button
            type="submit"
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-3 px-4 rounded-xl transition-all active:scale-[0.98] mt-2 shadow-lg shadow-emerald-500/20"
          >
            Iniciar Sesión
          </button>
        </form>
